...
public class Golf_FloatingScore : MonoBehaviour
{
    [Header("Set Dynamically")]
    public Golf_eFSState state = Golf_eFSState.idle;
    [SerializeField]
...
public GameObject reportFinishTo = null;
    ...
    public void Init(List<Vector2> ePts, float eTimeS = 0, float eTimeD = 1)
    {
        ...
        timeStart = eTimeS;
        timeDuration = eTimeD;
        state = Golf_eFSState.pre; // Set it to the pre state, ready to start moving
    }
    ...
    void Update()
    {
        ...
        if (state == Golf_eFSState.idle) return;
            ...
            { // If u>=1, we're done moving
                ...
                { // If there is nothing to callback
                  // ...then don't destroy this. Just let it stay still.
                    state = Golf_eFSState.idle;
                }
            }
            else
            {
                // 0<=u<1, which means that this is active and moving
                state = Golf_eFSState.active;
